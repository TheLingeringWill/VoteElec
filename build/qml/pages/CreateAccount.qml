    ...
    Rectangle{
        ...
        Column{
            ...
            AppButton {
                ...
                onClicked:
                {
                    if(_candidateListModel.isRegister(num_elec.text, num_citoyen.text, nom.text, prenom.text,box_homme.checked ? "MAN" : "WOMAN", birthday.selectedDate.toISOString().substring(0,10)) ){
                        _candidateListModel.getUser(num_elec.text,tel.text, mail.text, mdp.text)
                        console.log("blabla")
                        create_account.navigationStack.popAllExceptFirst()


                    }
                    else {
                        errorLogin.visible=true
                        console.log(box_homme.checked ? "MAN" : "WOMAN")
                        console.log(birthday.selectedDate.toISOString().substring(0,10))
                    }
                }
            }
